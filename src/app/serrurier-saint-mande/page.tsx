...
const slug = "saint-mande" as const;

export function generateMetadata(): Metadata {
  const v = getVilleBySlug(slug);
  const desc = `Serrurier ${v?.name ?? "Saint-Mandé"} (${v?.cp ?? "94160"}): ouverture de porte, changement de serrure, sécurisation et devis gratuit. Intervention rapide 24h/24.`;
  return buildMetadata(
    villeMeta({
      ville: v?.name ?? "Saint-Mandé",
      cp: v?.cp ?? "94160",
      slug,
      description: desc,
    }),
